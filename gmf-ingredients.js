...
function boot(){
 ...
 notice.textContent='Revisão da base genérica: consulte os campos conferidos e a fonte de cada item. Cadastros próprios e fichas de fornecedores permanecem conforme informados pela equipe.';
 ...
 list.querySelectorAll('tr').forEach(row=>{
  ...
  }else if(item && !['TACO','TBCA','IBGE','Rótulo'].includes(item.src)){
   note.textContent='Cadastro da equipe / fornecedor — mantido conforme informado. Referência cadastrada: '+(item.ref||'não informada');
  }else{note.textContent='Base genérica — referência pendente de conferência. Fonte declarada: '+(item&&item.ref||'não informada');}
