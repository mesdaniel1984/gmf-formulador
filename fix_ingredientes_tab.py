...
# Add Ingredientes button to nav
...
# Add functions before nextStep
old3 = 'function nextStep() {'
new3 = '''async function salvarIngrediente() {
...
async function carregarListaIngredientes() {
  ...
  if(!_sb){lista.innerHTML='<p style="color:var(--text3);font-size:13px">Faça login para ver ingredientes.</p>';return;}
  const {data,error}=await _sb.from('ingredientes_custom').select('nome,dados').order('nome');
  if(error||!data?.length){lista.innerHTML='<p style="color:var(--text3);font-size:13px">Nenhum ingrediente cadastrado ainda.</p>';return;}
  ...
  lista.innerHTML=`<table style="width:100%;border-collapse:collapse;font-size:12px">
    <tr style="background:#f8fafc"><th style="padding:8px;text-align:left;border-bottom:2px solid #e5e7eb">Nome</th><th style="padding:8px;text-align:left;border-bottom:2px solid #e5e7eb">Rótulo FTP</th><th style="padding:8px;text-align:center;border-bottom:2px solid #e5e7eb">kcal</th><th style="padding:8px;text-align:center;border-bottom:2px solid #e5e7eb">PTN</th><th style="padding:8px;text-align:center;border-bottom:2px solid #e5e7eb">CHO</th><th style="padding:8px;text-align:left;border-bottom:2px solid #e5e7eb">Fornecedor</th><th style="padding:8px;text-align:left;border-bottom:2px solid #e5e7eb">Por</th></tr>
    ${data.map(r=>`<tr style="border-bottom:1px solid #f3f4f6"><td style="padding:8px;font-weight:600">${r.dados.n}</td><td style="padding:8px;color:#6b7280">${r.dados.rotulo||'—'}</td><td style="padding:8px;text-align:center">${r.dados.kcal}</td><td style="padding:8px;text-align:center">${r.dados.ptn}</td><td style="padding:8px;text-align:center">${r.dados.cho}</td><td style="padding:8px;color:#6b7280">${r.dados.ref||'—'}</td><td style="padding:8px;color:#6b7280">${r.dados.cadastradoPor||'—'}</td></tr>`).join('')}
  </table>`;
}
function nextStep() {'''
...
new4 = '''<div class="panel" id="step6">
  <div style="max-width:900px;margin:0 auto;padding:24px">
    <div style="background:#fff;border-radius:12px;border:1px solid #e5e7eb;padding:24px;margin-bottom:20px">
      ...
      <p style="font-size:13px;color:#6b7280;margin-bottom:20px">Adicione novos ingredientes ao banco de dados. Após salvar ficam disponíveis para todos na tela de Receita.</p>
    ...
    <div style="background:#fff;border-radius:12px;border:1px solid #e5e7eb;padding:20px">
      <div style="font-size:14px;font-weight:700;color:#1e3a8a;margin-bottom:12px">📋 Ingredientes Cadastrados por Usuários</div>
